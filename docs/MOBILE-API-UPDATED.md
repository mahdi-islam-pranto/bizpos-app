...
Query: `q` (name, barcode, SKU, generic name), `lowOnly=1`, `trashed=1`, `page`,
`perPage`.

```json
{
  "data": [ { "id": 42, "name": "Napa 500mg", "barcode": "...", "purchasePrice": 1.0, "salePrice": 1.2,
              "wholesalePrice": 1.1, "mrp": 1.2, "minimumStock": 20, "vatPercent": 0, "isActive": true,
...
`trackBatch`. Leave `openingStock` out and the product arrives with **1** on the
shelf; send `0` for none.
...
`unit` (a `short` from `/catalog/lookups`), `mrp`, `vatPercent`,
`minimumStock`, `openingStock`, `reason`, `note` and `confirmedNew`.
